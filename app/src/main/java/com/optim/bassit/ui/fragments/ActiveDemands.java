...
public class ActiveDemands extends Fragment implements DemandsServiceAdapter.ItemClickListener {

    private View view;
    @Inject
    AppApi appApi;

    @BindView(R.id.recycler_active_demands)
    ...
    public ActiveDemands(){}

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.fragment_active_demands, container, false);
    ...
    private void refreshApi() {
        ...
        demandService.setState(1);
        if (CurrentUser.getInstance().isPro()){
            demandService.setPro_id(CurrentUser.getInstance().getmCustomer().getId());
            appApi.getDemandsServicePro(1, CurrentUser.getInstance().getmCustomer().getId()).enqueue(new Callback<List<DemandService>>() {
                @Override
            ...
            appApi.getDemandsServiceUser(1, CurrentUser.getInstance().getmCustomer().getId()).enqueue(new Callback<List<DemandService>>() {
                @Override
                public void onResponse(Call<List<DemandService>> call, Response<List<DemandService>> response) {
                    if (response.isSuccessful()) {
                        demandsServiceAdapter.fill(response.body());
    ...
    public static ActiveDemands newInstance(String text) {

        ActiveDemands f = new ActiveDemands();
        Bundle b = new Bundle();
